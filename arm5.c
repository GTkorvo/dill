...
static void
arm_pldsti(dill_stream c, int type, int ls, int dest, int src, long offset);
static void
arm_pldst(dill_stream c, int type, int ls, int dest, int src1, int src2);

static void
arm_bswap(dill_stream c, int type, int reg)
{
...
extern int
arm_local(dill_stream c, int type)
{
...
extern int
arm_localb(dill_stream c, int size)
{
    arm_mach_info ami = (arm_mach_info) c->p->mach_info;
    if (size < 0) size = 0;
...
extern void
arm_save_restore_op(dill_stream c, int save_restore, int type, int reg)
{
    arm_mach_info ami = (arm_mach_info) c->p->mach_info;
...
static void
arm_movi2f(dill_stream c, int dest, int src)
{
    arm_mach_info ami = (arm_mach_info) c->p->mach_info;
    arm_pstorei(c, DILL_I, 0, src, _fp, ami->conversion_word);
    arm_ploadi(c, DILL_F, 0, dest, _fp, ami->conversion_word);
}
    
static void
arm_movf2i(dill_stream c, int dest, int src)
{
    arm_mach_info ami = (arm_mach_info) c->p->mach_info;
    arm_pstorei(c, DILL_F, 0, src, _fp, ami->conversion_word);
    arm_ploadi(c, DILL_I, 0, dest, _fp, ami->conversion_word);
}
    
static void
arm_movd2i(dill_stream c, int dest, int src)
{
    arm_mach_info ami = (arm_mach_info) c->p->mach_info;
    arm_pstorei(c, DILL_D, 0, src, _fp, ami->conversion_word);
    if (ami->stack_align == 8) {
	arm_ploadi(c, DILL_L, 0, dest, _fp, ami->conversion_word);
    } else {
	arm_ploadi(c, DILL_I, 0, dest, _fp, ami->conversion_word);
	arm_ploadi(c, DILL_I, 0, dest+1, _fp, ami->conversion_word+4);
    }
}
    
static void
arm_movi2d(dill_stream c, int dest, int src)
{
    arm_mach_info ami = (arm_mach_info) c->p->mach_info;
    if (ami->stack_align == 8) {
	arm_pstorei(c, DILL_L, 0, src, _fp, ami->conversion_word);
    } else {
	arm_pstorei(c, DILL_I, 0, src, _fp, ami->conversion_word);
	arm_pstorei(c, DILL_I, 0, src+1, _fp, ami->conversion_word+4);
    }
    arm_ploadi(c, DILL_D, 0, dest, _fp, ami->conversion_word);
}
...
extern void
arm_proc_start(dill_stream c, char *subr_name, int arg_count, arg_info_list args,
	     dill_reg *arglist)
{
    int i;
    int max_in_reg = _a4;
    ...
    arm_nop(c);
    arm_nop(c);
    ami->conversion_word = arm_local(c, DILL_D);
    ami->conversion_word = arm_local(c, DILL_D);
    ami->conversion_word = arm_local(c, DILL_D);
	...
	if (i < 4) {
		...
		arm_pstorei(c, DILL_I, 0, args[i].in_reg, _fp, 
				    real_offset);
		args[i].in_reg = -1;
		args[i].out_reg = -1;
		...
		if ((args[i].type != DILL_F) && (args[i].type != DILL_D)) {
			...
			arm_pstorei(c, DILL_I, 0, args[i].in_reg, _fp, 
				    real_offset);
			arm_ploadi(c, DILL_F, 0, tmp_reg, _fp, real_offset);
			arm_ploadi(c, DILL_F, 0, tmp_reg+1, _fp, real_offset+4);
		    } else {
...
extern void arm_mod(dill_stream c, int sign, int type_long, int dest, 
		      int src1, int src2)
{
    int return_reg;
    if (sign == 1) {
	/* signed case */
	if (type_long) {
	    return_reg = dill_scalll(c, (void*)arm5_hidden_mod, "%l%l", src1, src2);
	    dill_movl(c, dest, return_reg);
	} else {
	    return_reg = dill_scalli(c, (void*)arm5_hidden_modi, "%i%i", src1, src2);
	    dill_movi(c, dest, return_reg);
	}
    } else {
	/* unsigned case */
	if (type_long) {
	    return_reg = dill_scalll(c, (void*)arm5_hidden_umod, "%l%l", src1, src2);
	    dill_movul(c, dest, return_reg);
	} else {
	    return_reg = dill_scallu(c, (void*)arm5_hidden_umodi, "%u%u", src1, src2);
	    dill_movu(c, dest, return_reg);
	}
    }
}

extern void arm_modi(dill_stream c, int data1, int data2, int dest, int src1, 
		      long imm)
{
    arm_set(c, _v1, imm);
...
extern void arm_div(dill_stream c, int unsign, int junk, int dest, int src1,
		      int src2)
{
    int return_reg;
    ...
    return_reg = dill_scalll(c, routine, "%l%l", src1, src2);
    dill_movl(c, dest, return_reg);
}

#define MUL(c,A,S,Rd,Rs,Rm) INSN_OUT(c, COND(AL)|(A&1)<<21|(S&1)<<20|RN(Rd)|RD(0)|(Rs&0xf)<<8|0x90|(Rm&0xf))

extern void arm_mul(dill_stream c, int unsign, int junk, int dest, int src1,
		      int src2)
{
    MUL(c, 0, 0, dest, src1, src2);
}

extern void arm_muli(dill_stream c, int unsign, int junk, int dest, int src,
		      long imm)
{
    arm_set(c, _v1, imm);
...
extern void arm_divi(dill_stream c, int unsign, int junk, int dest, int src, 
		      long imm)
{
    arm_set(c, _v1, imm);
...
static void
arm_saverestore_floats(dill_stream c, int saverestore)
{
    int i;
    for (i=1; i <8; i++) {
	if (dill_mustsave(&c->p->tmp_f, i)) {
	    arm_save_restore_op(c, saverestore, DILL_D, i);
	}
    }
}
...
extern void
arm_convert(dill_stream c, int from_type, int to_type, 
	      int dest, int src)
{
    from_type &= 0xf;
    to_type &= 0xf;
    ...
    case CONV(DILL_F,DILL_U):
	    ...
	    ret = dill_scallu(c, (void*)arm5_hidden_ftou, "%f", src);
	    arm_saverestore_floats(c, 1);
	    arm_mov(c, DILL_UL, 0, dest, ret);
	}
	break;
	/* fallthrough */
    case CONV(DILL_F,DILL_UL):
	    ...
	    int ret;
	    arm_saverestore_floats(c, 0);
	    ret = dill_scallul(c, (void*)arm5_hidden_ftoul, "%f", src);
	    arm_saverestore_floats(c, 1);
	    arm_mov(c, DILL_UL, 0, dest, ret);
	}
	break;
    ...
    case CONV(DILL_D,DILL_U):
	    ...
	    int ret;
	    arm_saverestore_floats(c, 0);
	    ret = dill_scallu(c, (void*)arm5_hidden_dtou, "%d", src);
	    arm_saverestore_floats(c, 1);
	    arm_mov(c, DILL_U, 0, dest, ret);
	}
	break;
    case CONV(DILL_D,DILL_UL):
	    ...
	    int ret;
	    arm_saverestore_floats(c, 0);
	    ret = dill_scallul(c, (void*)arm5_hidden_dtoul, "%d", src);
	    arm_saverestore_floats(c, 1);
	    arm_mov(c, DILL_UL, 0, dest, ret);
	}
	break;
    ...
    case CONV(DILL_UL,DILL_D): 
	    ...
	    int ret;
	    arm_saverestore_floats(c, 0);
	    ret = dill_scalld(c, (void*)arm5_hidden_ultod, "%l", src);
	    arm_saverestore_floats(c, 1);
	    arm_mov(c, DILL_D, 0, dest, ret);
	}
	break;
    ...
    case CONV(DILL_UL,DILL_F):
	    ...
	    int ret;
	    arm_saverestore_floats(c, 0);
	    ret = dill_scallf(c, (void*)arm5_hidden_ultof, "%l", src);
	    arm_saverestore_floats(c, 1);
	    arm_mov(c, DILL_D, 0, dest, ret);
	}
	break;
...
extern void 
arm_jal(dill_stream c, int return_addill_reg, int target)
{

}

extern void 
arm_jump_to_label(dill_stream c, unsigned long label)
{
    dill_mark_branch_location(c, label);
			  ...
			  void *value_ptr)
{
	...
	if (arg.is_immediate) {
		...
		arm_pstorei(c, DILL_I, 0, _v1, _sp, real_offset);
		arm_set(c, _v1, *(((int*)value_ptr)+1));
		arm_pstorei(c, DILL_I, 0, _v1, _sp, real_offset+4);
	    }		
	} else {
	...
	if ((type != DILL_F) && (type != DILL_D)) {
		...
		if (type == DILL_F) {
			...
			/* boundary condition */
			arm_movf2i(c, arg.out_reg, *(int*)value_ptr);
			arm_pstorei(c, DILL_F, 0, (*(int*)value_ptr)+1, _sp, 
				    real_offset + 4);
		    }
		}
	    }
...
static void
arm_simple_ret(dill_stream c)
{
    dill_mark_ret_location(c);
...
arm_setf(dill_stream c, int type, int junk, int dest, double imm)
{
	...
	b.d = imm;
	arm_set(c, _v1, b.i[0]);
	arm_pstorei(c, DILL_I, 0, _v1, _fp, ami->conversion_word);
	arm_set(c, _v1, b.i[1]);
	arm_pstorei(c, DILL_I, 0, _v1, _fp, ami->conversion_word+4);
	arm_ploadi(c, DILL_D, 0, dest, _fp, ami->conversion_word);
    }
}	
...
extern int
arm_init_disassembly_info(dill_stream c, void * ptr)
{
...
extern int
arm_print_insn(dill_stream c, void *info_ptr, void *insn)
{
#ifdef HAVE_PRINT_INSN_ARM
...
#else
extern int
arm_init_disassembly_info(dill_stream c, void * ptr){return 0;}
extern int arm_print_insn(dill_stream c, void *info_ptr, void *insn){return 0;}
#endif

extern void
arm_print_reg(dill_stream c, int typ, int reg)
{
    switch(typ) {
    case DILL_C: case DILL_UC:
...
extern int
arm_count_insn(dill_stream c, int start, int end)
{
    return (end - start)>>2;
}
